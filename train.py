...
def train(cfg):
	...
	if(cfg.params.cuda):
		...
		trainer = pl.Trainer(accelerator="gpu", devices=1, max_epochs=cfg.params.epoch, logger=tb_logger, callbacks=callbacks, log_every_n_steps=10) # 
	else:
		...
		trainer = pl.Trainer(max_epochs=cfg.params.epoch, logger=tb_logger, callbacks=callbacks, log_every_n_steps=10) # 
