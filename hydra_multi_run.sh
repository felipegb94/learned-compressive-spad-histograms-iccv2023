...
## STATUS: Resuming Training in compoptics
## Run CSPH1D TruncFourier K=16 (Increased Learning Rate Experiment)
...
python train_resume.py ++experiment=test_csph ++model.model_params.init=TruncFourier ++model.model_params.k=16 ++train_params.p_tv=0.0 ++train_params.workers=8 ++train_params.epoch=20 ++train_params.lri=1e-3 ++random_seed=1234
...
# ## Run CSPH1D HybridGrayFourier K=16 (Increased Learning Rate Experiment) 
...
# ## STATUS: DONE running in fmu
# ## Run CSPH1D HybridGrayFourier K=32 (Increased Learning Rate Experiment) 
