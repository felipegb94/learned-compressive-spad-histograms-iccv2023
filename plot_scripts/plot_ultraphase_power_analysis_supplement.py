...
if __name__=='__main__':
	...
	n_pixels = 4
	fps = 30
	out_fname = 'supplement_ultraphase_power_nphotons-{}_npixels-{}_fps-{}'.format(n_photons, n_pixels, fps)

	df = pd.DataFrame(columns=['name', 'n_pixels', "n_photon_timestamps_per_frame", "fps", "processing_power", "est_swisspad2_readout_power"])

	# ## Data for: 2x2 + 550 photons
	if(n_photons == 550 and n_pixels == 4 and fps == 30):
		fourier_k8_data = {
			"name": ["fourier_k8"]
			, "processing_power": [1.34E-08]
			, "est_swisspad2_readout_power": [8.36E-07]
		}
		hybrid_learned_fourier_k8_data = {
			"name": ["hybrid_learned_fourier_k8"]
			, "processing_power": [1.76E-08	]	
			, "est_swisspad2_readout_power": [8.36e-07]
		}

		fourier_k16_data = {
			"name": ["fourier_k16"]
			, "processing_power": [3.32E-08	]
			, "est_swisspad2_readout_power": [1.67E-06]
		}
		hybrid_learned_fourier_k16_data = {
			"name": ["hybrid_learned_fourier_k16"]
			, "processing_power": [3.98E-08]	
			, "est_swisspad2_readout_power": [1.67E-06]
		}

		coarsehist_k16_data = {
			"name": ["coarsehist_k16"]
			, "processing_power": [2.74E-10]
			, "est_swisspad2_readout_power": [8.36e-07]
		}
		coarsehist_k64_data = {
			"name": ["coarsehist_k64"]
			, "processing_power": [1.75E-10]
			, "est_swisspad2_readout_power": [3.35e-06]		
		}
		coarsehist_k256_data = {
			"name": ["coarsehist_k256"]
			, "processing_power": [1.75E-10	]
			, "est_swisspad2_readout_power": [1.34E-05]		
		}
	...
	fourier_k16_data = pd.DataFrame.from_dict(fourier_k16_data)
	hybrid_learned_fourier_k16_data = pd.DataFrame.from_dict(hybrid_learned_fourier_k16_data)
	...
	coarsehist_k256_data = pd.DataFrame.from_dict(coarsehist_k256_data)
	data = pd.concat([
		                fourier_k8_data, 
		                hybrid_learned_fourier_k8_data,
		                fourier_k16_data, 
	...
	plot_utils.update_fig_size(fig, height=2,width=16)
	width = 0.8
	df.plot( kind= 'bar', ax=ax , secondary_y= 'est_swisspad2_readout_power', rot=0, width=width, logy=True)
	ax.tick_params(
	...
	ax2 = plt.gca()
	plot_utils.set_ticks(ax2, fontsize=14)
