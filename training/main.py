...
def main():
    ...
    for epoch in range(start_epoch, opt["epoch"]):
        ...
        print("==================>Train<==================")
        print("KL: {}, TV: {}, RMSE: {}".format(\
            np.mean(train_loss["KL"][-(len(train_data)-1):]), \
                np.mean(train_loss["TV"][-(len(train_data)-1):]), \
                    np.mean(train_loss["RMSE"][-(len(train_data)-1):])))
        print("==================>Validation<==================")
        print("KL: {}, TV: {}, RMSE: {}".format(\
            np.mean(val_loss["KL"][-(len(train_data)//(opt["batch_size"]*opt["save_every"])-1):]), \
